...
const Items = () => {
  ...
  const handleAddItem = async () => {
    ...
    if (
      options.some(
        (opt) => !opt.name.trim() || opt.customizations.some((c) => !c.trim()),
      )
    ) {
  ...
  const handleEditItem = async (itemId) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/items/edititem/${itemId}`,
        {
          name,
          price: Number(price),
          options,
  ...
  const handleDeleteItem = async (itemId) => {
    ...
    try {
      await axios.delete(
        `${process.env.REACT_APP_BACKEND_URL}/items/deleteitem/${itemId}`,
      );
  ...
  return (
    <div className="font-body">
      ...
      <div className="relative bg-primary overflow-hidden">
        ...
        <div className="relative z-10 px-8 py-14">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <p className="text-on-primary/60 text-xs font-bold uppercase tracking-widest mb-2 font-label">
                The Masters Atelier
              </p>
              ...
              <p className="text-on-primary/70 mt-3 max-w-md">
                Curate your atelier's catalog — define garment types, pricing
                tiers, and customization options.
              </p>
            </div>
          ...
          <div className="flex gap-8 mt-8 pt-6 border-t border-white/10">
            <div>
              ...
              <p className="text-on-primary/60 text-xs font-label mt-0.5">
                Garment Types
              </p>
            </div>
            <div>
      ...
      <div className="p-8">
        ...
        {showForm && (
          <div
            className="bg-surface-container-lowest rounded-2xl p-8 mb-8"
              ...
              <div>
                ...
                <div className="space-y-4">
                  {options.map((option, optionIndex) => (
                    <div
                      key={optionIndex}
                      className="p-5 bg-surface-container-low rounded-xl"
                    >
        ...
        {filteredItems.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            <div className="w-20 h-20 rounded-3xl bg-surface-container-low flex items-center justify-center">
              <span className="material-symbols-outlined text-[40px] text-stone-300">
                content_cut
              </span>
            </div>
            <div className="text-center">
              ...
              <p className="text-sm text-stone-300 mt-1">
                {searchQuery
                  ...
                  : "Add your first garment type to get started"}
              </p>
            </div>
            {!searchQuery && (
              <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredItems.map((item, index) => (
              ...
              >
                {/* Card image area */}
                <div
                  className={`aspect-[3/2] bg-gradient-to-br ${cardColors[index % cardColors.length]} flex flex-col items-center justify-center relative`}
                >
                ...
                {/* Card body */}
                <div className="p-5">
                  <h3 className="font-extrabold text-on-surface font-headline text-lg leading-tight mb-1">
                    {item.name}
                  </h3>
                  ...
                  {expandedCard === item._id && (
                    <div className="mb-4 space-y-3">
                      {item.options.map((option, i) => (
                        <div key={i}>
                          <p className="text-[10px] font-bold uppercase tracking-wider text-stone-400 mb-1.5 font-label">
                            {option.name}
                          </p>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() =>
                        setExpandedCard(
                          expandedCard === item._id ? null : item._id,
                        )
                      }
            ...
            >
              ...
              <span className="text-sm font-bold text-stone-400 group-hover:text-primary transition-colors font-label">
                Add New Item
              </span>
            </button>
          </div>

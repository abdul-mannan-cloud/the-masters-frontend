...
const ProductTypeView = () => {
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      const { productType: updated } = await productTypeService.toggleProductTypeStatus(
        id,
        activating,
      );
  ...
  const sortedWorkflow = [...productType.workflow].sort((a, b) => a.sequence - b.sequence);
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3">
              ...
              <StatusBadge status={productType.isActive ? "active" : "inactive"} />
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            ...
            <div className="space-y-3 text-sm">
              <div>
                ...
                <p className="text-on-surface">{productType.description || "—"}</p>
              </div>
              <div>
          ...
          >
            ...
            {sortedMeasurements.length === 0 ? (
              <p className="text-sm text-on-surface-variant">No measurement fields defined.</p>
            ) : (
          ...
          >
            ...
            {productType.options.length === 0 ? (
              <p className="text-sm text-on-surface-variant">No options defined.</p>
            ) : (
              <div className="space-y-3">
                {productType.options.map((option) => (
                  <div key={option.name}>
                    <p className="text-sm font-bold text-on-surface mb-1.5">{option.name}</p>
          ...
          >
            ...
            {sortedWorkflow.length === 0 ? (
              <p className="text-sm text-on-surface-variant">No workflow steps defined.</p>
            ) : (
              <div className="space-y-2">
                {sortedWorkflow.map((step) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-on-surface-variant text-xs">{step.requiredSkill}</p>

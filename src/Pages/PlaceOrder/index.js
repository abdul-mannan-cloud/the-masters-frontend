...
const PlaceOrder = () => {
  ...
  const [isMeasurementFileUploaded, setIsMeasurementFileUploaded] =
    useState(false);
  ...
  const fetchNextOrderNumber = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/customer/next-order-number`,
      );
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      getCustomers(customersPage, searchQuery, searchBy);
    }, 300);
  ...
  const getCustomers = async (page = 1, query = "", field = "all") => {
    try {
      ...
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/customer/getallcustomers`,
        { params },
      );
  ...
  const getItems = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/items/getallitems`,
      );
  ...
  const fetchCustomerDetails = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/customer/get/${savedCustomer}`,
      );
  ...
  const handleAddCustomer = async () => {
    try {
      ...
      if (newCustomer._id) {
        ...
        await axios.put(
          `${process.env.REACT_APP_BACKEND_URL}/customer/update/${newCustomer._id}`,
          { measurements },
        );
      ...
      if (error.response?.status === 409) {
        const msg =
          error.response.data?.message || "Order number already exists";
  ...
  const handleOptionChange = (index, optionName, customization) => {
    const updated = [...products];
    const existing = updated[index].options.findIndex(
      (opt) => opt.name === optionName,
    );
    if (existing >= 0)
      updated[index].options[existing].customization = customization;
  ...
  return (
    <div className="p-8 font-body">
      ...
      {/* Progress Steps */}
      <div
        className="bg-surface-container-lowest rounded-2xl p-6 mb-8"
            ...
            {steps.map((s, index) => {
              ...
              return (
                <div key={index} className="flex flex-col items-center gap-3">
                  ...
                  <span
                    className={`text-xs font-bold font-label ${active || done ? "text-primary" : "text-stone-400"}`}
                  >
                    {s.label}
                  </span>
                </div>
      ...
      {step === 1 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div
            ...
            style={{ boxShadow: "0 12px 40px rgba(25,28,27,0.04)" }}
          >
            <h2 className="text-xs font-bold uppercase tracking-widest text-stone-400 mb-5 font-headline">
              New Customer
            </h2>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setCustomer({ ...customer, orderNumber: e.target.value });
                      setOrderNumberError("");
                    }}
                ...
                {orderNumberError && (
                  <p className="mt-1 text-xs text-error font-body">
                    {orderNumberError}
                  </p>
                )}
              </div>
                ...
                <div key={field.key}>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onInput={
                        field.key === "phone" ? formatPhoneNumber : undefined
                      }
                      onChange={(e) =>
                        setCustomer({
                          ...customer,
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        {c.orderNumber && (
                          <span className="px-1.5 py-0.5 bg-primary/10 text-primary text-[10px] font-bold rounded font-mono">
                            #{c.orderNumber}
                          </span>
                        )}
                    ...
                    <span className="material-symbols-outlined text-[18px] text-stone-300">
                      chevron_right
                    </span>
                  </div>
                ))
            ...
            {!customersLoading && customersPagination.total > 0 && (
              <div className="flex items-center justify-between pt-4 mt-2 border-t border-outline-variant/10">
                <span className="text-xs text-stone-400">
                  Page {customersPagination.page} of{" "}
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={
                      !(customersPagination.hasPrevPage ?? customersPage > 1)
                    }
                  ...
                  <button
                    ...
                    disabled={
                      !(
                        customersPagination.hasNextPage ??
                        customersPage < (customersPagination.totalPages || 1)
                      )
                    }
                    className="p-1.5 rounded-lg border border-outline-variant/20 hover:bg-surface-container-low disabled:opacity-30 transition-colors"
                  >
          ...
          <div className="space-y-6">
            {products.map((product, index) => (
              ...
              >
                ...
                {/* Type Selection */}
                <div className="mb-5">
                  <label className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider mb-3 font-label">
                    Select Item Type
                  </label>
                ...
                {product.type && (
                  <div className="mb-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {items
                        .find((item) => item.name === product.type)
                        ?.options.map((option) => (
                          <div key={option.name}>
                            <label className="block text-xs text-stone-400 mb-1 font-label">
                              {option.name}
                            </label>
                            <select
                              onChange={(e) =>
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      handleProductChange(index, "instructions", e.target.value)
                    }
                    rows={2}
      ...
      {step === 3 && (
        <div>
          <div
            ...
            style={{ boxShadow: "0 12px 40px rgba(25,28,27,0.04)" }}
          >
            <h2 className="text-xs font-bold uppercase tracking-widest text-stone-400 mb-6 font-headline">
              Measurements
            </h2>
            <MeasurementDetails
            ...
            <div className="flex justify-end gap-3 mt-6 pt-6 border-t border-outline-variant/10">
              ...
              <button
                onClick={() => {
                  const hasMeasurementFile =
                    isMeasurementFileUploaded ||
                    ...
                    <td>
                      <div className="flex flex-wrap gap-1">
                        {product.options.map((opt, idx) => (
                          <span
                            key={idx}
                            className="text-xs px-2 py-0.5 bg-surface-container-low rounded-full text-on-surface-variant font-bold"
                          >
            ...
            <div className="px-8 py-6 border-t border-outline-variant/10">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <p className="text-xs text-stone-400 font-label">
                    adjustable
                  </p>
                </div>
              </div>

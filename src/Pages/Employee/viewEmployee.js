    ...
    <div className="p-8 font-body">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-stone-400 mt-1 text-sm">
            Full details and performance overview.
          </p>
        </div>
      </div>
      ...
      <div className="bg-primary rounded-2xl p-8 mb-6 text-on-primary flex flex-col md:flex-row md:items-center gap-6">
        ...
        <div className="text-right">
          <p className="text-xs text-on-primary/60 uppercase tracking-wider font-label mb-1">
            Balance
          </p>
          <p
            className={`text-2xl font-extrabold font-headline ${employeeData.payment < 0 ? "text-primary-fixed" : "text-on-error"}`}
          >
            Rs. {Math.abs(employeeData.payment).toLocaleString()}
          </p>
          <p className="text-xs text-on-primary/60 mt-1 font-label">
            {employeeData.payment < 0 ? "Debit" : "Credit"}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        {/* Donut Chart */}
        <div
          className="md:col-span-2 bg-surface-container-lowest rounded-2xl p-6"
          style={{ boxShadow: "0 12px 40px rgba(25,28,27,0.04)" }}
        >
          <h3 className="text-xs font-bold uppercase tracking-widest text-stone-400 mb-4 font-headline">
            ...
            {products.map((item, index) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="font-bold text-sm text-on-surface truncate font-headline">
                    {item.type}
                  </p>
                  ...
                  <span
                    className={`inline-block mt-2 text-xs px-2 py-0.5 rounded-full font-bold ${
      ...
      {modalIsOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center animate-backdrop-in">
          ...
          />
          <div
            className="relative bg-surface-container-lowest rounded-2xl max-w-lg w-full mx-4 animate-modal-in"
            style={{ boxShadow: "0 24px 60px rgba(25,28,27,0.15)" }}
          >
            <ShirtDetails
              item={selectedItem}
              model={modalIsOpen}
